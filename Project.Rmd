...
Mollusca %>% select(is.numeric)%>% shapiro_test_df()
```
...
ggplot(Mollusca, aes(x = Age, y = Whole_weight, col = Sex), na.rm = TRUE) +
  geom_boxplot()
ggplot(Mollusca, aes(x = Age, y = Diameter, col = Sex),na.rm = TRUE )+
  geom_boxplot()
...
mod_Mollusca <- get_pc(Mollusca[,3:9])
mod_Mollusca$Sex <- Mollusca$Sex
mod_Mollusca$Age <- Mollusca$Age
mod <- as.data.frame(mod_Mollusca)

ggplot(mod, aes(PC1, PC2),
  group=interaction(Age, Sex)) + 
  geom_point(col = "green") + geom_smooth()+facet_wrap(~Sex+Age, scales = "free")

res_pca <- prcomp(Mollusca[,3:9], center = TRUE, scale = TRUE)
summary(res_pca)
screeplot(res_pca, type = "b", npcs = 8, main = "Screeplot of the PCs")
