...
PkgNames <- c("dplyr", "magrittr", "ggplot2", "reshape2", "purrr","utf8", "data.table", "plyr", "rcompanion", "corrplot", "car", "multcomp", "DescTools", "vegan", "randomForest", "caTools", "ROCR", "rattle", "rpart", "gdata", "stringr", "plotly", "Rtsne")
...
str(mice)
```
В первой переменной записаны номера мышей в исследовании и очевидно, что там есть птехнические повторы, то разделим эту переменную на 2. ID ,первая переменная будет содержать ID мыши, вторая - номер повтора. Выведем количество получившихся мышей, всего их 72. Сгруппируем наши наблюдения по номеру мыши и посмотрим, сколько технических повторов приходится на каждую мышь, их 15.

```{r }
mice_number <- str_split(mice$MouseID, pattern = "_", n=2, simplify = T)
mice$ID <- as.factor(mice_number[,1])
mice$replicate <- as.factor(mice_number[,2])
...
c-SC-m - c-CS-m == 0 -0.0482717  0.0053980  -8.942  < 0.001 *** - обучение влияло на уровень BDNF_N у контрольных мышей, получавших мемантин.
c-SC-s - c-CS-s == 0 -0.0289228  0.0056900  -5.083  < 0.001 *** - обучение влияло на уровень BDNF_N у контрольных мышей, получавших раствор.
t-SC-s - t-CS-s == 0  0.020126   0.006129   3.284  0.02328   *  - обучение влияло на уровень BDNF_N у трансгенных мышей, получавших раствор.

t-CS-m - c-CS-m == 0 -0.0264852  0.0055459  -4.776  < 0.001 *** - генотип влиял на уровень BDNF_N у обученных мышей, получавших мемантин.
t-CS-s - c-CS-s == 0 -0.0368549  0.0060829  -6.059  < 0.001 *** - генотип влиял на уровень BDNF_N у обученных мышей, получавших раствор. 
t-SC-m - c-SC-m == 0  0.0301176  0.0055459   5.431  < 0.001 *** - генотип влиял на уровень BDNF_N у необученных мышей, получавших мемантин.
t-SC-s - c-SC-s == 0  0.018423   0.005912   3.116  0.03884   *  - генотип влиял на уровень BDNF_N у необученных мышей, получавших раствор.

Таким образом, можно сделать следующие выводы:
1. Обучение повышало уровень BDNF_N у  контрольных мышей вне зависимости от введения мемантина. У трансгенных животных обучение снижало BDNF_N только у животных, получавших раствор.
2. У трансгенных обученных мышей, уровень BDNF_N был ниже по сравнению с контрольными обученными мышами, вне зависимости от воздействия. В то же время, у необученых трансгенных животных уровень BDNF_N был выше чем у необученных контрольных животных, вне зависимости от воздействия. 
3. Сам по себе мемантин не оказывал значимого воздействия. 
...
Анализ главных компонент. Отцентрируем данныеи проведем PCA. В результате, получается что достаточным для описания наших данных являются первые 6 компонент. Первая компонента объясняет 27,89 % вариации в данных, PC2 - 44,25 %, PC3 - 55,37%, PC4 - 63,08 %, PC5 - 67,96 %, PC6 - 72,07%. Построим графики ординаций и факторных нагрузок. А также график распределения наших данных в осях первых 3 компонент. 
...
```{r}
colors <-  rainbow(length(unique(clean_data$class)))
names(colors) <-  unique(clean_data$class)
ecb = function(x,y){ plot(x,t='n'); text(x,labels=clean_data$class, col=colors[clean_data$class]) }
tsne_prot = tsne(clean_data[,2:72], epoch_callback = ecb, perplexity=100)


prot <- data.matrix(clean_data)
# perform dimensionality redcution from 64D to 2D
tsne <- Rtsne(as.matrix(prot[,2:72]), check_duplicates = FALSE, pca = FALSE, perplexity=30, theta=0.5, dims=2)

# display the results of t-SNE
cols <- rainbow(10)
plot(tsne$Y, t='n')
text(tsne$Y, labels=prot[,2:72], col=cols[prot[,2:72] +1])
